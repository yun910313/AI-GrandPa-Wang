    ...
    async findLatest(elderlyId: string): Promise<any | null> {
        ...
        try {
            ...
            const result = await pool.request()
                ...
                .query('SELECT TOP 1 id, elderly_id, latitude, longitude, address, CAST(timestamp AS VARCHAR) as timestamp FROM gps_logs WHERE elderly_id = @elderlyId ORDER BY timestamp DESC');
